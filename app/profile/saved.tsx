...
export default function SavedContentScreen() {
  ...
  const savedPlaces = useMemo(
    () => PLACES.filter((place) => SAVED_PLACE_IDS.includes(place.id)),
    []
  );
  const savedPromos = useMemo(
    () => PROMOS.filter((promo) => SAVED_PROMO_IDS.includes(promo.id)),
    []
  );
  const savedSessions = useMemo(
    () => SESSIONS.filter((session) => SAVED_SESSION_IDS.includes(session._id)),
    []
  );
    ...
    <>
      ...
      <View style={{ flex: 1, backgroundColor: THEME.bg }}>
        <AppHeader
          ...
          showLogo={false}
          title="Saved"
          subtitle="Places, promos, sessions"
        />
        <ScrollView
          style={{ flex: 1 }}
